...
const HomeScreen = ({ navigation }) => {
  ...
  return (
    <View style={styles.homeContainer}>
      <Image source={require('./assets/images/bookAndPencil.png')} style={styles.imageHomePage}/>
...
const ListScreen = ({ navigation, route }) => {
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://6454008bc18adbbdfead590d.mockapi.io/api/v1/api_todolist/${id}`, {
        method: 'DELETE',
      });
  ...
  <View style={styles.item}>
    ...
    <View style={{ flexDirection: 'row' }}>
      <TouchableOpacity onPress={onEdit}>
        <Icon name="pencil" size={20} color="#00BDD6" />
      </TouchableOpacity>
      <TouchableOpacity onPress={onDelete} style={{ marginLeft: 10 }}>
        <Icon name="times" size={20} color="#00BDD6" />
      </TouchableOpacity>
    </View>
  </View>
);
...
const AddScreen = ({ navigation, route }) => {
  ...
  const handleFinish = async () => {
    if (item) {
      // Cập nhật mục
      try {
        await fetch(`https://6454008bc18adbbdfead590d.mockapi.io/api/v1/api_todolist/${item.id}`, {
          method: 'PUT',
          headers: {
      ...
      // Thêm mục mới
      try {
        await fetch('https://6454008bc18adbbdfead590d.mockapi.io/api/v1/api_todolist', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
